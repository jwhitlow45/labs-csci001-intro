...
//The discriminant tells us how many times the given quadratic equation crosses the x-axis (y = 0).
...
//function prototypes
string userInput();
double validInput(bool), calcDiscrim(double, double, double), calcSolution(double, double, double, bool), checkSolution(double, double, double);
...
double checkSolution(double a, double b, double c) {
	...
	double x = calcSolution(a, b, c, true);

	double check = (a * pow(x, 2)) + (b * x) + c;								//Puts a, b, c, and x into a quadratic equation which if equal to 0 means that x is the correct solution
	...
	if (check == 0) {															//If x does not equal zero then it is not the solution and the calculation with the other form of the 
		...
		return calcSolution(a, b, c, false);
...
void printResult(double a, double b, double c) {
	...
	if (discrim > 0) {															//If the discriminant is > 0 then there are two real solutions and there is no need to check
		...
		cout << "There are two real solutions which are x = " << fixed << setprecision(2) << calcSolution(a, b, c, true) << " and x = " << calcSolution(a, b, c, false) << ".\n\n";
		...
		cout << "There is one real solution which is x = " << fixed << setprecision(2) << checkSolution(a, b, c) << ".\n\n";
